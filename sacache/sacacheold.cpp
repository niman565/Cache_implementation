...
int main(int argc, char** argv) {
	int lines = 4;
	const int spots = 8;	//cache lines = cache size/line bytes
	const int set = 8;
	int address, tag, data, func, index, place, lru, temp, big,setnum,line;
	bool dirty, hit;
...
if(!hit){
		...
		if (func == 0) { //func stands for 0x00/read or OxFF/write
			...
			output << std::hex<<std::uppercase << setfill('0')<<setw(2)<<(int) (sa[index][temp].data[address&7]&0xFF)<<' '<<hit<<' '<<dirty<<"\n";
		}
		else {
